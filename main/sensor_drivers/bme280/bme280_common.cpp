...
int BME280::getCalibrateData()
{
    ...
    int8_t digH4H5Shared = static_cast<int8_t>(
        readByteData(0xE5)); // this register hold parts of the values of dig_H4 and dig_h5
    int8_t digH5Msb = static_cast<int8_t>(readByteData(0xE6));
    dig_h6 = static_cast<int8_t>(readByteData(0xE7));

    dig_h4 = static_cast<signed short>(
        digH4Msb << 4 | (digH4H5Shared & 0x0F)); // split and shift the bits appropriately.
    dig_h5 = static_cast<signed short>(
        digH5Msb << 4 | ((digH4H5Shared & 0xF0) >> 4)); // split and shift the bits appropriately.
    //=======================================================================================
...
esp_err_t BME280::Init(const uint8_t humidityOversampling, const uint8_t temperatureOversampling,
                       const uint8_t pressureOversampling, const uint8_t sensorMode)
{
    ...
    status |= writeByteData(CTRL_MEAS, _pressureOversamplingValue | _temperatureOversamplingValue |
                                           _sensorModeValue);
...
esp_err_t BME280::SetTemperatureOversampling(
    const uint8_t tempOversampling) // ctrl_meas bits 7, 6, 5   page 29
{
...
esp_err_t
BME280::SetPressureOversampling(const uint8_t pressureOversampling) // ctrl_meas bits 4, 3, 2
{
...
esp_err_t BME280::SetOversampling(const uint8_t tempOversampling,
