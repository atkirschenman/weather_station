...
esp_err_t Spi::transferMultiplesBytes(const uint8_t reg_addr, uint8_t* tx_buf, uint8_t* rx_buf,
                                      size_t data_length, const uint8_t command)
{
...
esp_err_t Spi::Init(const spi_host_device_t spi_peripheral, const int pin_miso, const int pin_mosi,
                    const int pin_sclk)
{
...
esp_err_t Spi::RegisterDevice(const uint8_t mode, const int ss, const int addr_length,
                              const int command_length, const int bus_speed)
{
    esp_err_t status = ESP_OK;
...
esp_err_t Spi::WriteRegisterMultipleBytes(const uint8_t reg_addr, uint8_t* reg_data_buffer,
                                          const uint8_t byte_count, const uint8_t command)
{
    return transferMultiplesBytes(reg_addr, reg_data_buffer, nullptr, byte_count, command);
}

esp_err_t Spi::ReadRegisterMultipleBytes(const uint8_t reg_addr, uint8_t* reg_data_buffer,
                                         const uint8_t byte_count, const uint8_t command)
{
